...
const char * const BITCOIN_CONF_FILENAME = "asiadevelopcoin.conf";
const char * const BITCOIN_PID_FILENAME = "asiadevelopcoin.pid";
...
#else
    const char* pszModule = "asiadevelopcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AsiaDevelopCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AsiaDevelopCoin";
#else
    // Unix
    return pathRet / ".asiadevelopcoin";
#endif
#endif
}
