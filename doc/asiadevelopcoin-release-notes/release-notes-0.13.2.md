AsiaDevelopCoin Core version 0.13.2 is now available from:

  <https://download.asiadevelopcoin.org/asiadevelopcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/XGoldCoin/asiadevelopcoin/issues>
...
updates will be released anymore. Without security updates, using a asiadevelopcoin
...
ECDSA signatures inside AsiaDevelopCoin transactions now use validation using
...
Previous versions of AsiaDevelopCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of AsiaDevelopCoin Core was to raise the default minimum
relay fee.

AsiaDevelopCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
AsiaDevelopCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running asiadevelopcoind without having to do any manual configuration.
...
AsiaDevelopCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. AsiaDevelopCoin Core will relay transactions with
...
entirely in AsiaDevelopCoin Core 0.13, and it is at this time undecided whether the
...
AsiaDevelopCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
AsiaDevelopCoin Core automatically creates a hidden service to listen on, without
manual configuration. AsiaDevelopCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if AsiaDevelopCoin Core is listening, and
...
AsiaDevelopCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals AsiaDevelopCoin
Core to use floating fees. By default, AsiaDevelopCoin Core will use floating
fees.
...
At all times, AsiaDevelopCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ADC.
Furthermore, AsiaDevelopCoin Core will never create transactions paying less than
...
`asiadevelopcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /asiadevelopcoinrpc>
        ProxyPass http://127.0.0.1:31135/
        ProxyPassReverse http://127.0.0.1:31135/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass asiadevelopcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< asiadevelopcoinrpc:<password>
    </Location>
...
Restarting asiadevelopcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in asiadevelopcoin.conf
...
asiadevelopcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/asiadevelopcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The AsiaDevelopCoin Core code base has
...
- `asiadevelopcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `asiadevelopcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the AsiaDevelopCoin scripting system that in combination with
...
AsiaDevelopCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of AsiaDevelopCoin Core.
...
The two phases are distinct in the AsiaDevelopCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming AsiaDevelopCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for AsiaDevelopCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each AsiaDevelopCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of asiadevelopcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of asiadevelopcoins being spent is exactly
...
- **Increased security for multisig:** AsiaDevelopCoin addresses (both P2PKH addresses
  ...
  believed to be provided by AsiaDevelopCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of AsiaDevelopCoin's current ledger (called the UTXO set),
  ...
  note that AsiaDevelopCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  AsiaDevelopCoin users to individually opt-in to almost any change in the AsiaDevelopCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and AsiaDevelopCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the AsiaDevelopCoin
client to AsiaDevelopCoin Core 0.13.2 to signal for segwit support. When a
...
Since AsiaDevelopCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `asiadevelopcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `asiadevelopcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
